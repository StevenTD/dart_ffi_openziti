...
final DynamicLibrary dylib = DynamicLibrary.open(
  Platform.isWindows
      ? "lib/assets/win/lib/ziti.dll"
      : throw UnsupportedError("This plugin currently only supports Windows."),
);
...
class DartFfiOpenziti {
////////////////// Define function bindings/////////////////
  final ziti_lib_init =
      dylib.lookupFunction<Void Function(), void Function()>('Ziti_lib_init');

  final ziti_lib_shutdown = dylib
      .lookupFunction<Void Function(), void Function()>('Ziti_lib_shutdown');
  ...
  final ziti_listen = dylib.lookupFunction<Int32 Function(Int32, Int32),
      int Function(int, int)>('Ziti_listen');
  ...
  late final ZitiConnectAddrDart ziti_connect_addr =
      dylib.lookupFunction<ZitiConnectAddrC, ZitiConnectAddrDart>(
          'Ziti_connect_addr');

  final ZitiResolveDart _zitiResolve =
      dylib.lookupFunction<ZitiResolveC, ZitiResolveDart>('Ziti_resolve');

  final ZitiCheckSocketDart zitiCheckSocket =
      dylib.lookupFunction<ZitiCheckSocketC, ZitiCheckSocketDart>(
          'Ziti_check_socket');
  ...
  Future<String?> getPlatformVersion() {
    return DartFfiOpenzitiPlatform.instance.getPlatformVersion();
  }
  ...
  Future<String> zitiVersion() async {
    ...
    final String finalVersion = 'Ziti Version: $version, Revision: $revision';
    return finalVersion;
  }
